...
struct PrefixTree {
    ...
    void AddWord(const string& word) {
        ...
        curr->word = word;
        while (curr != nullptr) ++curr->count, curr = curr->parent;
    }
    void PrefixTreeDecCount(PrefixTree* curr) {
        while (curr != nullptr) --curr->count, curr = curr->parent;
    }
};
...
class Solution {
   ...
   public:
    ...
    void dfs(vector<vector<char>>& board, int x, int y, PrefixTree* curr, unordered_set<string>& ret) {
        char ch = board[x][y];
        if (curr->children.count(ch) == 0 || curr->count == 0) return;
        curr = curr->children[ch];
        board[x][y] = '#';
        if (!curr->word.empty()) ret.emplace(curr->word), curr->word.clear(), curr->PrefixTreeDecCount(curr);
        int m = board.size(), n = board[0].size();
