   ...
   public:
    int countSpecialNumbers(int n) {
        ...
        function<int(int, int, bool, bool)> f = [&](int pos, int mask, bool limit,
            ...
            if (!limit && started && memo[pos][mask] != -1) return memo[pos][mask];
            int ret = 0;
            if (!started) ret += f(pos + 1, mask, false, false);
            int up = limit ? s[pos] - '0' : 9;
            for (int d = 1 - started; d <= up; ++d) {
                if (((mask >> d) & 1) == 0) {
                    ret += f(pos + 1, mask | (1 << d), limit && d == up, true);
                }
            }
            if (!limit && started) memo[pos][mask] = ret;
            return ret;
        };
