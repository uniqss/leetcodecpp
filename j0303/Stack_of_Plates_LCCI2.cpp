...
class StackOfPlates {
    vector<pair<stack<int>, int>> m_stks;
    int m_cap;
   ...
   public:
    ...
    void push(int val) {
        if (m_cap <= 0) return;
        if (m_stks.empty() || m_stks[m_stks.size() - 1].second >= m_cap) {
            stack<int> stk;
            stk.push(val);
            m_stks.push_back({stk, 1});
        } else {
            m_stks[m_stks.size() - 1].first.push(val);
            ++m_stks[m_stks.size() - 1].second;
        }
    }

    int pop() {
        if (m_cap <= 0) return -1;
        if (m_stks.empty()) return -1;
        auto& it = m_stks[m_stks.size() - 1];
        auto ret = it.first.top();
        it.first.pop();
        if (--it.second == 0) {
            m_stks.pop_back();
        }
    ...
    int popAt(int index) {
        if (m_cap <= 0) return -1;
        if (m_stks.empty()) return -1;
        if (index < 0 || index >= m_stks.size()) return -1;
        auto& it = m_stks[index];
        auto ret = it.first.top();
        it.first.pop();
        if (--it.second == 0) {
            m_stks.erase(m_stks.begin() + index);
        }
        return ret;
    }
};
...
int main() {
    ...
    test({"StackOfPlates", "pop",   "pop",   "popAt", "popAt", "pop",   "push",  "push",  "push",  "push", "pop",
         ...
         {null, -1, -1,   -1,   -1,   -1,   null, null, null, null, 44,   null, null, -1,  42,   24,   null,
          -1,   42, null, 29,   44,   10,   40,   null, 56,   -1,   -1,   null, -1,   -1,  null, -1,   -1,
          null, 63, -1,   -1,   -1,   52,   -1,   -1,   null, -1,   null, null, 52,   -1,  -1,   null, -1,
          null, 17, 20,   null, null, null, -1,   -1,   30,   -1,   6,    43,   null, null});
